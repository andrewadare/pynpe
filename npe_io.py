...
def matrices():
    dcamat = dcamatrices()
    eptmat = eptmatrix()
    gpt, gptx, gptbins = genpt()

    # Divide columns by generated pt distribution
    eptmat /= gpt
    for (i, m) in enumerate(dcamat):
        dcamat[i] /= gpt
    return eptmat, dcamat
...
def dcabins():
    ...
    # ~ <pt> in bin TODO: do it right
    dcaeptx = dcaeptbins[:-1] + 0.4 * binwidths(dcaeptbins)
...
def genpt(opt=''):
    ...
    '''
    hpte, hptd, gptx, gptbins = hadronpt()
    N = hpte.shape[0] / 2 + 1
    ...
    b = h2a(h5)
    cx = binctrs(h4)
    ch, bins = np.histogram(cx, bins=gptbins[:N], weights=c)
    bh, bins = np.histogram(cx, bins=gptbins[:N], weights=b)
